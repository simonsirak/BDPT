...
void FindPathHelper(Intersection& i, vec3 directionToPoint, int currentDepth, int maxDepth, vector<Intersection>& intersections);
...
void FindPath(vec3 start, vec3 direction, int maxDepth, vector<Intersection>& intersections){
	...
	FindPathHelper(i, direction, 1, maxDepth, intersections);
}

void FindPathHelper(Intersection& point, vec3 directionToPoint, int currentDepth, int maxDepth, vector<Intersection>& intersections){
	if(currentDepth <= maxDepth){
		...
		vec3 normalToPoint = triangles[point.triangleIndex]->normal(point.position);
		...
		if(ClosestIntersection(point.position, newDirection, triangles, i)){
			intersections.push_back(i);
			FindPathHelper(point, newDirection, currentDepth+1, maxDepth, intersections);
		}else{
...
vec3 calcRadianceToPoint(const BDPTpath& path, unsigned int i) {
	...
	if(i >= path.intersections.size()){
		//cout << triangles[path.endP.triangleIndex]->emission << endl;
		return vec3(0, 0, 0);
	}else{
		Intersection p0;
		Intersection p1 = path.intersections[i];
		Intersection p2;

		if(i == 0){
			p0 = path.startP;
		}else{
			p0 = path.intersections[i-1];
		}

		if(i == path.intersections.size() - 1){
			p2 = path.endP;
		}else{
			p2 = path.intersections[i+1];
		}

		// vec3 n1 = triangles[path.intersections[i].triangleIndex]->normal(p1);
		...
		// cout << incoming.x << " " << incoming.y << " " << incoming.z << endl;
		...
		// //cout << triangles[path.intersections[i].triangleIndex]->emission << endl;
		// vec3 res = emittance + (BRDF * incoming * cos_theta / p);
		// cout << res.x << " " << res.y << " " << res.z << endl;

		vec3 newstart = p1.position;
		vec3 start = p0.position;
		vec3 newdir = glm::normalize(p2.position - p1.position);

		vec3 surfaceToLight = glm::normalize(start - newstart);

		vec3 normal = glm::dot(surfaceToLight, triangles[p2.triangleIndex]->normal(newstart)) * triangles[p2.triangleIndex]->normal(newstart) / glm::dot(triangles[p2.triangleIndex]->normal(newstart), triangles[p2.triangleIndex]->normal(newstart));
		...
		float cos_theta = glm::dot(newdir, normal); // note that the cosine factor is part of the Rendering Equation integrand/differential, NOT the BRDF.
		vec3 BRDF = triangles[p2.triangleIndex]->color / float(PI); // just the reflectance of the material / PI for Lambertian Reflection: http://www.oceanopticsbook.info/view/surfaces/lambertian_brdfs
